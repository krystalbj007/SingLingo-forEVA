...
export const LyricsView: React.FC<LyricsViewProps> = ({ 
  ...
  useEffect(() => {
    if (containerRef.current) {
        ...
        if (activeEl) {
            // Calculate center position manually for better precision
            const container = containerRef.current;
            const containerHeight = container.clientHeight;
            const elTop = activeEl.offsetTop;
            const elHeight = activeEl.offsetHeight;
            
            // Desired scroll position: Element center aligns with Container center
            // scrollTop = (elTop + elHeight/2) - containerHeight/2
            // We add a small offset (e.g. -20px) to bias it slightly upwards visually
            const targetScroll = (elTop + elHeight / 2) - (containerHeight / 2);
  ...
  }, [currentLineIndex, lines]); // Added lines dependency in case translation updates affect layout

  return (
    <div className="flex-1 w-full overflow-y-auto no-scrollbar py-[40vh] scroll-pt-48 relative z-0 perspective-1000" ref={containerRef}>
        ...
        {lines.map((line, index) => {
          ...
          return (
            <motion.div
              ...
              className={`text-center cursor-pointer transition-all duration-500 rounded-2xl w-full max-w-3xl relative overflow-hidden group break-words 
                ${isActive 
            ...
            >
               ...
               {(isAnalyzing || (isActive && !line.analysis)) && (
              ...
              <AnimatePresence>
                {isActive && (
                    ...
                    <div className="inline-block relative px-2">
                        ...
                        <p className="text-slate-400 text-sm md:text-lg font-light tracking-wider leading-relaxed break-words">
